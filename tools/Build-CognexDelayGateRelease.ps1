...
$releaseRoot = Join-Path $repoRoot ("artifacts\\releases\\mvf-cognex-auto-trigger-dark-filter-" + $Version)
...
try {
    ...
    $readmeSource = Join-Path $repoRoot "real-world-projects\docs\cognex-auto-trigger-release.md"
...
'@
    [System.IO.File]::WriteAllText((Join-Path $releaseRoot "run-cognex-auto-trigger.ps1"), $runScript, [System.Text.Encoding]::ASCII)
