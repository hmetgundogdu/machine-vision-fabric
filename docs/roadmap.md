...
| M2.5 | **Snapshot + module-state recovery** (checkpoint/restore module state, worker-crash supervision, resume-after-crash) | 🟢 **C.1 done.** Module state is captured/restored through a shared-memory slot (**no base64**); `SupervisedWorker` (`IWorkerChannel`) transparently restarts a crashed child, restores its last state, and retries; the **executor checkpoints stateful workers at cycle boundaries** (`--checkpoint-every N`) so a live run always has a recent last-good. Real-python: a killed worker recovers mid-run and its frame count survives (2→crash→3→4). Next: **C.2 engine-crash resume** (persist the engine cursor + reopen the file-backed state region on restart) |
