...
| M2 | Our own **shared-memory** zero-copy data plane, graph-aware (no network) — see `data-plane-design.md` | 🟡 Slices A + B done (engine-owned arena behind `IDataPlane`; graph-aware publish-once fan-out with static-graph refcounts). **B.3 in progress:** generic **typed-payload** plane — DLPack-style `PayloadDescriptor` written into the slot header, engine-validated; the arena/seam/classifier carry `(descriptor, bytes)` and the Python SDK wraps the payload as a **zero-copy** `memoryview`/numpy array. **base64 fully removed** (arena required). Real-python cross-language round-trip green. Next: worker transformer (frame-out) + snapshot |
