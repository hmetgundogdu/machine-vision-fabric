...
| EG | **Realtime egress** — async **frame-data publish** (live preview / local stream) + **frame-state publish** (per-frame decisions/measurements/metrics) out to a studio or optional observer; best-effort, **off the hot path** (Telemetry Rule) | ⬜ **Not yet designed** — concept only; needs a design pass (see Open decisions). Design-gated: discuss before building |
...
- **Picking mid-run (2026-07-23).** A `select` whose criterion is its own binding is now a live tunable,
  ...
  carries.
- **Realtime egress — async frame-data + frame-state publish (EG, not yet designed).** A live pipeline should
  ...
  - **Open questions:** publish granularity (every frame vs sampled/rate-limited); who owns the encode
    ...
    publishing its last state; how a subscriber discovers what a running graph exposes. **Needs a design pass
