...
| M2 | Our own **shared-memory** zero-copy data plane, graph-aware (no network) — see `data-plane-design.md` | 🟡 Slices A + B done — engine-owned arena behind the `IDataPlane` seam; graph-aware routing publishes a fan-out frame **once** with a refcount from the static graph (workers share one copy; in-process consumers keep the heap frame). Transformer (worker frame output) + snapshot next |
