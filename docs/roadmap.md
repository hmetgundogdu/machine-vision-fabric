...
| EG | **Realtime egress** — async **frame-data publish** (live preview / local stream) + **frame-state publish** (per-frame decisions/measurements/metrics) out to a studio or optional observer; best-effort, **off the hot path** (Telemetry Rule) | 🟢 **Done.** Full design in [`realtime-egress-design.md`](realtime-egress-design.md): one framed binary record (state + frame-data via the arena's own `PayloadDescriptor`), `IEgressSink` seam in core, bounded-ring drop off the hot path, selectable **TCP / WebSocket / UDP** transports, UDP alive-beacon discovery, TS (`clients/mvf-egress-client`, incl. browser) + .NET (`Mvf.Egress.Client`) **consumer SDKs**, and a separate-page TUI egress view. Slices 0–5 all ✅; verified cross-language over TCP + WS + UDP. Producer: `src/egress/Mvf.Egress`. |
...
- **Picking mid-run (2026-07-23).** A `select` whose criterion is its own binding is now a live tunable,
  ...
  carries.
- **Realtime egress — async frame-data + frame-state publish (EG, designed → see [`realtime-egress-design.md`](realtime-egress-design.md); slice 1 in progress).** A live pipeline should
  be able to *emit while it runs* — the actual **frame data** (for a live preview or a local stream, e.g. the
  studio UI or a MediaMTX bridge) and the **frame state** (per-frame decisions, measurements, node stats) for
  observability or an optional central observer. This is a **concept, not a design** yet; capturing the shape
  of the problem so it is not lost:
  - **Off the hot path, always.** Per the Telemetry Rule egress is **optional, non-blocking, best-effort** —
    the executor must never wait on a publish. A slow or absent consumer drops, it does not stall the graph
    (its own backpressure surface, distinct from the arena's Stall/Drop). This is *publish*, not a data edge:
    it never participates in refcounts or ordering guarantees.
  - **Two shapes, maybe two channels.** Frame *state* is small structured messages (fits the existing stdio
    control-plane style, or a local socket the runtime doesn't otherwise use). Frame *data* is large and
    already lives in the shared-memory arena — an egress consumer might read a slot in place (zero-copy)
    rather than get a copy, mirroring how downstream nodes read frames.
  - **The `no-network` boundary.** The runtime's data/control planes stay strictly local (no TCP/gRPC). Egress
    to a *remote* observer necessarily leaves the machine, so it must be a **separate, clearly-bounded, opt-in
    channel** that the core never depends on — the same "adapter behind a seam" shape as `IDataPlane` /
    `IOutOfProcessModuleHost`, not a hole in the constraint. WebSocket is the first-choice transport per the
    product direction; MediaMTX/local streaming is the first-choice media bridge.
  - **Open questions:** publish granularity (every frame vs sampled/rate-limited); who owns the encode
    (JPEG/PNG for preview) and where it runs so it doesn't tax the hot path; whether a paused graph keeps
    publishing its last state; how a subscriber discovers what a running graph exposes. **Design pass done
    (2026-08-19) — resolved in [`realtime-egress-design.md`](realtime-egress-design.md):** publish every
    accepted cycle; frame-data forwards the arena's `PayloadDescriptor` + bytes near-zero-copy (no re-encode
    on the hot path); a paused graph keeps beaconing (status `paused`) but stops streaming; subscribers
    discover via the UDP alive-beacon.
