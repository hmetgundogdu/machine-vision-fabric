...
| M2.5 | **Snapshot + module-state recovery** (checkpoint/restore module state, worker-crash supervision, resume-after-crash) | 🟡 C.1 done — checkpoint/restore of module state through a shared-memory slot (**no base64**); `SupervisedWorker` (`IWorkerChannel`) transparently restarts a crashed child, restores its last state, and retries. Real-python: a killed worker recovers mid-run and its frame count survives (2→crash→3). Still to wire: **executor-driven periodic checkpoints** at cycle boundaries (so a live run always has a recent last-good), then **C.2 engine-crash resume** (persist cursor + reopen the file-backed state) |
