...
| M2 | Our own **shared-memory** zero-copy data plane, graph-aware (no network) — see `data-plane-design.md` | 🟡 Slices A + B done (engine-owned arena behind `IDataPlane`; graph-aware publish-once fan-out with static-graph refcounts). **B.3 in progress:** generic **typed-payload** plane (DLPack-style `PayloadDescriptor` in the slot header, engine-validated; SDKs wrap bytes as native objects zero-copy, no base64) — descriptor foundation done. Then transformer + snapshot |
